...
public class SpatialIndex {
     ...
     * @param boundary 空域边界
     */
    public SpatialIndex(AirspaceBoundary boundary) {
        if (boundary == null) {
     ...
     * @param position 位置
     */
    public void insert(String entityId, Position position) {
     ...
     */
    public void update(String entityId, Position newPosition) {
        if (entityId == null || entityId.trim().isEmpty()) {
            throw new IllegalArgumentException("实体ID不能为空");
        }
        if (newPosition == null) {
     ...
     * @return 范围内的实体ID列表
     */
    public List<String> queryRange(Position center, double radius) {
        if (center == null) {
     ...
     * @return 区域内的实体ID列表
     */
    public List<String> queryBox(double minX, double minY, double minZ,
     ...
     * @return 边界内的实体ID列表
     */
    public List<String> getEntitiesInBoundary() {
        return entityPositions.entrySet().stream()
                .filter(entry -> boundary.contains(entry.getValue()))
                .map(Map.Entry::getKey)
                .collect(Collectors.toList());
    }
    ...
    public String toString() {
        return String.format("SpatialIndex{entityCount=%d, boundary=%s}", 
                           entityPositions.size(), boundary);
    }
} 